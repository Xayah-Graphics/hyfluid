...
namespace ngp::train {
    ...
    public:
        explicit InstantNGP(const NerfDatasetLike auto& dataset, const TrainOptions& options = {}) {
            try {
                if (options.test_frame_limit == 0u) throw std::runtime_error{"test_frame_limit must be positive."};
                if (options.test_output_dir.empty()) throw std::runtime_error{"test output directory must not be empty."};
                ...
                std::size_t training_pixel_bytes = 0uz;
                for (const auto& frame : dataset.train) training_pixel_bytes += frame.rgba.size();
                host_pixels.reserve(training_pixel_bytes);
                host_camera.reserve(frame_count * 17uz);
                ...
                for (const auto& frame : dataset.train) {
                    ...
                    if (static_cast<float>(frame.focal_x) <= 0.0f || static_cast<float>(frame.focal_y) <= 0.0f || !std::isfinite(static_cast<float>(frame.principal_x)) || !std::isfinite(static_cast<float>(frame.principal_y))) throw std::runtime_error{"training frame camera intrinsics are invalid."};

                    host_pixels.append_range(frame.rgba);
                    for (std::size_t i = 0uz; i < 12uz; ++i) host_camera.push_back(static_cast<float>(frame.camera[i]));
                    host_camera.push_back(static_cast<float>(frame.time));
                    host_camera.push_back(static_cast<float>(frame.focal_x));
                    host_camera.push_back(static_cast<float>(frame.focal_y));
                    host_camera.push_back(static_cast<float>(frame.principal_x));
                    host_camera.push_back(static_cast<float>(frame.principal_y));
                }
                ...
                if (validation_frame_count != 0uz) {
                    ...
                    validation_camera.reserve(validation_frame_count * 17uz);
                    ...
                    for (const auto& frame : dataset.validation) {
                        ...
                        if (static_cast<float>(frame.focal_x) <= 0.0f || static_cast<float>(frame.focal_y) <= 0.0f || !std::isfinite(static_cast<float>(frame.principal_x)) || !std::isfinite(static_cast<float>(frame.principal_y))) throw std::runtime_error{"validation frame camera intrinsics are invalid."};
                        ...
                        validation_camera.push_back(static_cast<float>(frame.time));
                        validation_camera.push_back(static_cast<float>(frame.focal_x));
                        validation_camera.push_back(static_cast<float>(frame.focal_y));
                        validation_camera.push_back(static_cast<float>(frame.principal_x));
                        validation_camera.push_back(static_cast<float>(frame.principal_y));
                    }
                ...
                if (test_frame_count != 0uz) {
                    ...
                    test_camera.reserve(test_frame_count * 17uz);
                    ...
                    for (const auto& frame : dataset.test) {
                        ...
                        if (static_cast<float>(frame.focal_x) <= 0.0f || static_cast<float>(frame.focal_y) <= 0.0f || !std::isfinite(static_cast<float>(frame.principal_x)) || !std::isfinite(static_cast<float>(frame.principal_y))) throw std::runtime_error{"test frame camera intrinsics are invalid."};
                        ...
                        test_camera.push_back(static_cast<float>(frame.time));
                        test_camera.push_back(static_cast<float>(frame.focal_x));
                        test_camera.push_back(static_cast<float>(frame.focal_y));
                        test_camera.push_back(static_cast<float>(frame.principal_x));
                        test_camera.push_back(static_cast<float>(frame.principal_y));
                    }
                ...
                this->host.density_param_count  = cuda::config::MLP_WIDTH * cuda::config::GRID_OUTPUT_WIDTH + (cuda::config::DENSITY_HIDDEN_LAYERS - 1u) * cuda::config::MLP_WIDTH * cuda::config::MLP_WIDTH + cuda::config::NETWORK_OUTPUT_WIDTH * cuda::config::MLP_WIDTH;
                this->host.mlp_param_count      = this->host.density_param_offset + this->host.density_param_count;
                ...
                for (std::uint32_t level = 0u; level < cuda::config::GRID_N_LEVELS; ++level) {
                    ...
                    const std::uint64_t dense_params_in_level = static_cast<std::uint64_t>(resolution) * resolution * resolution * resolution;
                ...
                this->host.global_color_param_offset                 = this->host.grid_param_offset + this->host.grid_param_count;
                this->host.global_color_param_count                  = 3u;
                this->host.total_param_count                         = this->host.global_color_param_offset + this->host.global_color_param_count;
                ...
                cuda::initialize_mlp_parameters(this->host.density_param_offset, this->device.params_full_precision, this->device.params, this->device.param_gradients);
                ...
                cuda::initialize_global_color_parameters(this->host.global_color_param_offset, this->device.params_full_precision, this->device.params, this->device.param_gradients);
            } catch (...) {
    ...
    private:
        struct HostData {
            ...
            std::uint32_t global_color_param_offset                                 = 0u;
            std::uint32_t global_color_param_count                                  = 0u;
