...
namespace ngp::cuda {
    ...
    namespace {
        ...
        inline constexpr std::uint32_t SAMPLE_COORD_FLOATS                    = 8u;
        ...
        __device__ std::uint32_t coherent_prime_hash(const std::uint32_t x, const std::uint32_t y, const std::uint32_t z, const std::uint32_t t) {
            return x ^ (y * 2654435761u) ^ (z * 805459861u) ^ (t * 3674653429u);
        }

        __device__ std::uint32_t grid_index(const std::uint32_t hashmap_size, const std::uint32_t resolution, const std::uint32_t x, const std::uint32_t y, const std::uint32_t z, const std::uint32_t t) {
            const std::uint64_t dense_size = static_cast<std::uint64_t>(resolution) * resolution * resolution * resolution;
            if (dense_size <= hashmap_size) {
                const std::uint64_t index = static_cast<std::uint64_t>(x) + static_cast<std::uint64_t>(resolution) * (static_cast<std::uint64_t>(y) + static_cast<std::uint64_t>(resolution) * (static_cast<std::uint64_t>(z) + static_cast<std::uint64_t>(resolution) * t));
                return static_cast<std::uint32_t>(index % hashmap_size);
            }
            return coherent_prime_hash(x, y, z, t) % hashmap_size;
        }
        ...
        __global__ void mark_untrained_density_grid_kernel(float* __restrict__ density_grid_values, const std::uint32_t frame_count, const std::uint32_t width, const std::uint32_t height, const float focal_x, const float focal_y, const float principal_x, const float principal_y, const float* __restrict__ camera) {
            ...
            for (std::uint32_t frame = 0u; frame < frame_count && !trained; ++frame) {
                const float* frame_camera     = camera + static_cast<std::uint64_t>(frame) * CAMERA_FLOATS;
                ...
                const float frame_focal_x     = frame_camera[13u];
                const float frame_focal_y     = frame_camera[14u];
                const float frame_principal_x = frame_camera[15u];
                const float frame_principal_y = frame_camera[16u];
                ...
                for (std::uint32_t corner = 0u; corner < 8u && !trained; ++corner) {
                    ...
                    const float pixel_x = local_x * frame_focal_x / local_z + frame_principal_x;
                    const float pixel_y = local_y * frame_focal_y / local_z + frame_principal_y;
                    if (pixel_x <= 0.0f || pixel_y <= 0.0f || pixel_x >= static_cast<float>(width) || pixel_y >= static_cast<float>(height)) continue;

                    const float ray_x    = (pixel_x - frame_principal_x) / frame_focal_x;
                    const float ray_y    = (pixel_y - frame_principal_y) / frame_focal_y;
        ...
        __global__ void generate_density_grid_samples_kernel(const std::uint32_t sample_count, Pcg32 rng, const std::uint32_t density_grid_ema_step, const float threshold, const float* __restrict__ density_grid_values, float* __restrict__ sample_coords, std::uint32_t* __restrict__ density_grid_indices) {
            ...
            coord[3]                = rng.next_float();
            coord[4]                = MIN_CONE_STEPSIZE;
            coord[5]                = 0.5f;
            coord[6]                = 0.5f;
            coord[7]                = 0.5f;
        ...
        __global__ void generate_training_samples_kernel(const std::uint32_t rays_per_batch, const std::uint32_t sample_limit, const std::uint32_t current_step, const std::uint32_t frame_count, const std::uint32_t width, const std::uint32_t height, const float focal_x, const float focal_y, const float principal_x, const float principal_y, const float* __restrict__ camera, const std::uint8_t* __restrict__ occupancy, std::uint32_t* __restrict__ ray_counter, std::uint32_t* __restrict__ sample_counter, std::uint32_t* __restrict__ ray_indices_out, float* __restrict__ rays_out, std::uint32_t* __restrict__ numsteps_out, float* __restrict__ coords_out) {
            ...
            const float* frame_camera     = camera + static_cast<std::uint64_t>(image) * CAMERA_FLOATS;
            const float frame_time        = frame_camera[12u];
            const float frame_focal_x     = frame_camera[13u];
            const float frame_focal_y     = frame_camera[14u];
            const float frame_principal_x = frame_camera[15u];
            const float frame_principal_y = frame_camera[16u];
            ...
            const float ray_x       = (u * static_cast<float>(width) - frame_principal_x) / frame_focal_x;
            const float ray_y       = (v * static_cast<float>(height) - frame_principal_y) / frame_focal_y;
            const float3 camera_x   = {frame_camera[0], frame_camera[1], frame_camera[2]};
            const float3 camera_y   = {frame_camera[3], frame_camera[4], frame_camera[5]};
            const float3 camera_z   = {frame_camera[6], frame_camera[7], frame_camera[8]};
            ...
            while (j < numsteps) {
                ...
                if (is_density_grid_occupied(pos, occupancy)) {
                    ...
                    coord[3]     = frame_time;
                    coord[4]     = dt;
                    coord[5]     = warped_direction.x;
                    coord[6]     = warped_direction.y;
                    coord[7]     = warped_direction.z;
        ...
        __global__ void generate_evaluation_samples_kernel(const std::uint32_t tile_pixels, const std::uint32_t pixel_offset, const std::uint32_t max_samples, const std::uint32_t width, const std::uint32_t height, const float focal_x, const float focal_y, const float principal_x, const float principal_y, const float* __restrict__ evaluation_camera, const std::uint32_t evaluation_image_index, const std::uint8_t* __restrict__ occupancy, std::uint32_t* __restrict__ sample_counter, std::uint32_t* __restrict__ overflow_counter, std::uint32_t* __restrict__ numsteps_out, float* __restrict__ coords_out) {
            ...
            const float* frame_camera        = evaluation_camera + static_cast<std::uint64_t>(evaluation_image_index) * CAMERA_FLOATS;
            const float frame_time           = frame_camera[12u];
            const float frame_focal_x        = frame_camera[13u];
            const float frame_focal_y        = frame_camera[14u];
            const float frame_principal_x    = frame_camera[15u];
            ...
            const float ray_x       = (static_cast<float>(pixel_x) + 0.5f - frame_principal_x) / frame_focal_x;
            const float ray_y       = (static_cast<float>(pixel_y) + 0.5f - frame_principal_y) / frame_focal_y;
            const float3 camera_x   = {frame_camera[0], frame_camera[1], frame_camera[2]};
            const float3 camera_y   = {frame_camera[3], frame_camera[4], frame_camera[5]};
            const float3 camera_z   = {frame_camera[6], frame_camera[7], frame_camera[8]};
            const float3 ray_origin = {frame_camera[9], frame_camera[10], frame_camera[11]};
        ...
        __global__ void accumulate_evaluation_loss_kernel(const std::uint32_t tile_pixels, const std::uint32_t pixel_offset, const std::uint32_t evaluation_image_index, const std::uint32_t width, const std::uint32_t height, const std::uint8_t* __restrict__ evaluation_pixels, const std::uint32_t* __restrict__ numsteps_in, const float* __restrict__ coords_in, const __half* __restrict__ network_output, double* __restrict__ evaluation_loss_sum, std::uint8_t* __restrict__ comparison_pixels) {
            ...
            if (i < tile_pixels) {
                ...
                for (std::uint32_t j = 0u; j < numsteps; ++j) {
                    ...
                    const float alpha   = 1.0f - __expf(-density * coord[4u]);
        ...
        __global__ void encode_grid_forward_kernel(const std::uint32_t sample_count, const std::uint32_t grid_offset_0, const std::uint32_t grid_offset_1, const std::uint32_t grid_offset_2, const std::uint32_t grid_offset_3, const std::uint32_t grid_offset_4, const std::uint32_t grid_offset_5, const std::uint32_t grid_offset_6, const std::uint32_t grid_offset_7, const std::uint32_t grid_offset_8, const float* __restrict__ sample_coords, const __half* __restrict__ grid, __half* __restrict__ encoded_positions) {
            ...
            grid_position_fraction(sample[3], pos_t, grid_t, scale);
            ...
            for (std::uint32_t corner = 0u; corner < 16u; ++corner) {
                ...
                const float weight        = (high_x ? pos_x : 1.0f - pos_x) * (high_y ? pos_y : 1.0f - pos_y) * (high_z ? pos_z : 1.0f - pos_z) * (high_t ? pos_t : 1.0f - pos_t);
                const std::uint32_t index = grid_index(hashmap_size, resolution, high_x ? grid_x + 1u : grid_x, high_y ? grid_y + 1u : grid_y, high_z ? grid_z + 1u : grid_z, high_t ? grid_t + 1u : grid_t) * config::GRID_FEATURES_PER_LEVEL;
        ...
        __global__ void encode_grid_backward_kernel(const std::uint32_t sample_count, const std::uint32_t grid_offset_0, const std::uint32_t grid_offset_1, const std::uint32_t grid_offset_2, const std::uint32_t grid_offset_3, const std::uint32_t grid_offset_4, const std::uint32_t grid_offset_5, const std::uint32_t grid_offset_6, const std::uint32_t grid_offset_7, const std::uint32_t grid_offset_8, const float* __restrict__ sample_coords, const __half* __restrict__ encoded_position_gradients, __half* __restrict__ grid_gradients) {
            ...
            for (std::uint32_t corner = 0u; corner < 16u; ++corner) {
                ...
                const bool high_t         = (corner & 8u) != 0u;
                const float weight        = (high_x ? pos_x : 1.0f - pos_x) * (high_y ? pos_y : 1.0f - pos_y) * (high_z ? pos_z : 1.0f - pos_z) * (high_t ? pos_t : 1.0f - pos_t);
                const std::uint32_t index = grid_index(hashmap_size, resolution, high_x ? grid_x + 1u : grid_x, high_y ? grid_y + 1u : grid_y, high_z ? grid_z + 1u : grid_z, high_t ? grid_t + 1u : grid_t) * config::GRID_FEATURES_PER_LEVEL + feature;
                const __half weight_half  = weight;
        ...
        __global__ void compose_global_color_output_kernel(const std::uint32_t batch_size, const __half* __restrict__ density_output, const __half* __restrict__ params, const std::uint32_t global_color_param_offset, __half* __restrict__ network_output) {
            const std::uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
            if (i >= batch_size) return;
            network_output[static_cast<std::uint64_t>(i) * MLP_OUTPUT_WIDTH + 0u] = params[global_color_param_offset + 0u];
            network_output[static_cast<std::uint64_t>(i) * MLP_OUTPUT_WIDTH + 1u] = params[global_color_param_offset + 1u];
            network_output[static_cast<std::uint64_t>(i) * MLP_OUTPUT_WIDTH + 2u] = params[global_color_param_offset + 2u];
            network_output[static_cast<std::uint64_t>(i) * MLP_OUTPUT_WIDTH + 3u] = density_output[i];
            for (std::uint32_t channel = 4u; channel < MLP_OUTPUT_WIDTH; ++channel) network_output[static_cast<std::uint64_t>(i) * MLP_OUTPUT_WIDTH + channel] = static_cast<__half>(0.0f);
        }
        ...
        __global__ void accumulate_global_color_gradients_kernel(const std::uint32_t batch_size, const __half* __restrict__ network_output_gradients, __half* __restrict__ gradients, const std::uint32_t global_color_param_offset) {
            const std::uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
            if (i >= batch_size) return;
            atomicAdd(gradients + global_color_param_offset + 0u, network_output_gradients[static_cast<std::uint64_t>(i) * MLP_OUTPUT_WIDTH + 0u]);
            atomicAdd(gradients + global_color_param_offset + 1u, network_output_gradients[static_cast<std::uint64_t>(i) * MLP_OUTPUT_WIDTH + 1u]);
            atomicAdd(gradients + global_color_param_offset + 2u, network_output_gradients[static_cast<std::uint64_t>(i) * MLP_OUTPUT_WIDTH + 2u]);
        }
    ...
    void initialize_mlp_parameters(const std::uint32_t density_param_offset, float* const params_full_precision, std::uint16_t* const params, std::uint16_t* const param_gradients) {
        ...
        const std::uint32_t mlp_param_count = density_param_offset + DENSITY_NETWORK_PARAMS;
        std::vector host_params(mlp_param_count, 0.0f);
        Pcg32 rng{config::TRAIN_SEED};

        const float input_scale = std::sqrt(6.0f / static_cast<float>(config::MLP_WIDTH + MLP_INPUT_WIDTH));
        for (std::uint32_t i = 0u; i < config::MLP_WIDTH * MLP_INPUT_WIDTH; ++i) host_params[density_param_offset + i] = rng.next_float() * 2.0f * input_scale - input_scale;

        const float output_scale = std::sqrt(6.0f / static_cast<float>(MLP_OUTPUT_WIDTH + config::MLP_WIDTH));
        for (std::uint32_t i = 0u; i < MLP_OUTPUT_WIDTH * config::MLP_WIDTH; ++i) host_params[density_param_offset + MLP_FIRST_LAYER_PARAMS + i] = rng.next_float() * 2.0f * output_scale - output_scale;
    ...
    void initialize_global_color_parameters(const std::uint32_t global_color_param_offset, float* const params_full_precision, std::uint16_t* const params, std::uint16_t* const param_gradients) {
        if (params_full_precision == nullptr || params == nullptr || param_gradients == nullptr) throw std::runtime_error{"invalid global color parameter initialization input."};

        const std::array<float, 3> host_color = {0.4054651081081644f, 0.4054651081081644f, 0.4054651081081644f};
        if (const cudaError_t status = cudaMemcpy(params_full_precision + global_color_param_offset, host_color.data(), host_color.size() * sizeof(float), cudaMemcpyHostToDevice); status != cudaSuccess) throw std::runtime_error{std::string{"cudaMemcpy global color params failed: "} + cudaGetErrorString(status)};
        if (const cudaError_t status = cudaMemset(param_gradients + global_color_param_offset, 0, host_color.size() * sizeof(__half)); status != cudaSuccess) throw std::runtime_error{std::string{"cudaMemset global color gradients failed: "} + cudaGetErrorString(status)};
        cast_params_to_half_kernel<<<1u, THREADS_PER_BLOCK>>>(static_cast<std::uint32_t>(host_color.size()), params_full_precision + global_color_param_offset, reinterpret_cast<__half*>(params + global_color_param_offset));
        if (const cudaError_t status = cudaGetLastError(); status != cudaSuccess) throw std::runtime_error{std::string{"cast_params_to_half_kernel global color failed: "} + cudaGetErrorString(status)};
    }
    ...
    void evaluate_network(const std::uint32_t sample_count, const float* const sample_coords, const std::uint32_t* const grid_offsets, const std::uint16_t* const params, const std::uint32_t density_param_offset, const std::uint32_t global_color_param_offset, const std::uint32_t grid_param_offset, std::uint16_t* const density_input, std::uint16_t* const rgb_input, std::uint16_t* const network_output) {
        ...
        for (std::uint32_t offset = 0u; offset < sample_count; offset += config::NETWORK_BATCH_SIZE) {
            ...
            compose_global_color_output_kernel<<<linear_blocks, THREADS_PER_BLOCK>>>(chunk, reinterpret_cast<const __half*>(rgb_input), reinterpret_cast<const __half*>(params), global_color_param_offset, reinterpret_cast<__half*>(network_output) + static_cast<std::uint64_t>(offset) * MLP_OUTPUT_WIDTH);
            if (const cudaError_t status = cudaGetLastError(); status != cudaSuccess) throw std::runtime_error{std::string{"compose_global_color_output_kernel inference failed: "} + cudaGetErrorString(status)};
        }
    }
        ...
        for (std::uint32_t evaluation_image_offset = 0u; evaluation_image_offset < evaluation_image_count; ++evaluation_image_offset) {
            ...
            for (std::uint32_t pixel_offset = 0u; pixel_offset < total_pixels; pixel_offset += VALIDATION_TILE_RAYS) {
                ...
                if (used_samples > 0u) {
                    ...
                    evaluate_network(padded_used_samples, sample_coords, grid_offsets, params, density_param_offset, global_color_param_offset, grid_param_offset, density_input, rgb_input, network_output);
                }
    ...
    void forward_network(const float* const sample_coords, const std::uint32_t* const grid_offsets, const std::uint16_t* const params, const std::uint32_t density_param_offset, const std::uint32_t global_color_param_offset, const std::uint32_t grid_param_offset, std::uint16_t* const density_input, std::uint16_t* const rgb_input, std::uint16_t* const density_forward_hidden, std::uint16_t* const rgb_forward_hidden, std::uint16_t* const network_output) {
        ...
        compose_global_color_output_kernel<<<linear_blocks, THREADS_PER_BLOCK>>>(config::NETWORK_BATCH_SIZE, reinterpret_cast<const __half*>(rgb_input), reinterpret_cast<const __half*>(params), global_color_param_offset, reinterpret_cast<__half*>(network_output));
        if (const cudaError_t status = cudaGetLastError(); status != cudaSuccess) throw std::runtime_error{std::string{"compose_global_color_output_kernel failed: "} + cudaGetErrorString(status)};
    }
    ...
    void backward_network(const float* const sample_coords, const std::uint32_t* const grid_offsets, const std::uint16_t* const params, std::uint16_t* const gradients, const std::uint32_t density_param_offset, const std::uint32_t global_color_param_offset, const std::uint32_t grid_param_offset, const std::uint16_t* const density_input, const std::uint16_t* const rgb_input, const std::uint16_t* const density_forward_hidden, const std::uint16_t* const rgb_forward_hidden, const std::uint16_t* const network_output, const std::uint16_t* const network_output_gradients, std::uint16_t* const rgb_output_gradients, std::uint16_t* const rgb_input_gradients, std::uint16_t* const density_input_gradients, std::uint16_t* const density_backward_hidden, std::uint16_t* const rgb_backward_hidden, void* const cublaslt_handle, std::uint8_t* const cublaslt_workspace) {
        ...
        if (const cudaError_t status = cudaMemset(rgb_input_gradients, 0, static_cast<std::size_t>(MLP_INPUT_WIDTH) * config::NETWORK_BATCH_SIZE * sizeof(__half)); status != cudaSuccess) throw std::runtime_error{std::string{"cudaMemset density output gradients failed: "} + cudaGetErrorString(status)};
        if (const cudaError_t status = cudaMemset(gradients + global_color_param_offset, 0, 3u * sizeof(__half)); status != cudaSuccess) throw std::runtime_error{std::string{"cudaMemset global color gradients failed: "} + cudaGetErrorString(status)};
        accumulate_global_color_gradients_kernel<<<linear_blocks, THREADS_PER_BLOCK>>>(config::NETWORK_BATCH_SIZE, reinterpret_cast<const __half*>(network_output_gradients), reinterpret_cast<__half*>(gradients), global_color_param_offset);
        if (const cudaError_t status = cudaGetLastError(); status != cudaSuccess) throw std::runtime_error{std::string{"accumulate_global_color_gradients_kernel failed: "} + cudaGetErrorString(status)};
