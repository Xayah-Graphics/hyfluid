...
namespace hyfluid::train {
    void HyFluid::initialize(const DatasetView& dataset) {
        try {
            ...
            // ====================================================================================================
            {
                ...
                if (dataset.sim_to_world.size() != config::transform_value_count) throw std::runtime_error{"sim_to_world must contain 16 values."};
                if (dataset.world_to_sim.size() != config::transform_value_count) throw std::runtime_error{"world_to_sim must contain 16 values."};
                if (dataset.voxel_scale.size() != config::vector3_value_count) throw std::runtime_error{"voxel_scale must contain 3 values."};
                if (dataset.render_center.size() != config::vector3_value_count) throw std::runtime_error{"render_center must contain 3 values."};
                for (std::size_t i = 0uz; i < config::transform_value_count; ++i) {
                ...
                for (std::size_t i = 0uz; i < config::vector3_value_count; ++i) {
            ...
            // ====================================================================================================
            {
                ...
                for (const FrameSetView& frame_set : dataset.frame_sets) {
                    ...
                    if (static_cast<std::uint64_t>(frame_set.view_count) * frame_set.time_count > std::numeric_limits<std::uint32_t>::max()) throw std::runtime_error{std::format("frame set '{}' view-time grid is too large.", frame_set.name)};
                    ...
                    if (first_frame.height > std::numeric_limits<std::uint64_t>::max() / static_cast<std::uint64_t>(first_frame.width) / config::rgba_channel_count) throw std::runtime_error{std::format("frame set '{}' contains an image that is too large.", frame_set.name)};
                    const std::uint64_t frame_pixel_bytes = static_cast<std::uint64_t>(first_frame.width) * first_frame.height * config::rgba_channel_count;
                    ...
                    camera.reserve(frame_set.frames.size() * config::camera_value_count);
                    intrinsics.reserve(frame_set.frames.size() * config::intrinsics_value_count);
                    ...
                    for (const FrameView& frame : frame_set.frames) {
                        ...
                        const std::uint64_t pixel_offset = static_cast<std::uint64_t>(pixels.size());
                    ...
                    this->host.frame_sets.push_back(HostFrameSet{
                        ...
                        .pixel_bytes       = static_cast<std::uint64_t>(pixels.size()),
                        .camera_values     = static_cast<std::uint64_t>(camera.size()),
                        .intrinsics_values = static_cast<std::uint64_t>(intrinsics.size()),
                    });
                }
            }
            ...
            // ====================================================================================================
            {
                ...
                for (const VideoView& video : dataset.videos) {
                    ...
                    for (std::uint32_t i = 0u; i < this->host.frame_sets.size(); ++i) {
                    ...
                    if (video.camera.size() != config::camera_value_count) throw std::runtime_error{std::format("video '{}' contains {} camera values; expected {}.", video.file_name, video.camera.size(), config::camera_value_count)};
                    ...
                    host_video.frame_set = std::string{video.frame_set};
                    host_video.file_name = std::string{video.file_name};
                    for (std::size_t i = 0uz; i < config::camera_value_count; ++i) host_video.camera[i] = video.camera[i];
                    host_video.frame_set_index = frame_set_index;
                    host_video.width           = video.width;
                    host_video.height          = video.height;
                    host_video.frame_count     = video.frame_count;
                    host_video.frame_rate      = video.frame_rate;
                    host_video.view_index      = video.view_index;
                    host_video.focal           = video.focal;
                    this->host.videos.push_back(std::move(host_video));
                }
                for (std::uint32_t frame_set_index = 0u; frame_set_index < this->host.frame_sets.size(); ++frame_set_index) {
                    const HostFrameSet& frame_set = this->host.frame_sets[frame_set_index];
                    if (video_count_by_frame_set[frame_set_index] != frame_set.view_count) throw std::runtime_error{std::format("frame set '{}' contains {} videos; expected {}.", frame_set.name, video_count_by_frame_set[frame_set_index], frame_set.view_count)};
                    for (const std::uint8_t seen : video_seen_by_frame_set[frame_set_index])
                        if (seen == 0u) throw std::runtime_error{std::format("frame set '{}' is missing video metadata.", frame_set.name)};
                }
            }
    ...
    std::expected<OptimizationStats, std::string> HyFluid::optimize(const OptimizationRequest request) {
        (void) request;
        return std::unexpected{std::string{"optimize is not implemented in the dynamic HyFluid training framework."}};
    }

    std::expected<EvaluationStats, std::string> HyFluid::evaluate(const EvaluationRequest request) const {
        (void) request;
        return std::unexpected{std::string{"evaluate is not implemented in the dynamic HyFluid training framework."}};
    }

    std::expected<void, std::string> HyFluid::export_weights(const std::filesystem::path& path) const {
        (void) path;
        return std::unexpected{std::string{"export_weights is not implemented in the dynamic HyFluid training framework."}};
    }

    std::expected<void, std::string> HyFluid::load_weights(const std::filesystem::path& path) {
        (void) path;
        return std::unexpected{std::string{"load_weights is not implemented in the dynamic HyFluid training framework."}};
    }
} // namespace hyfluid::train
