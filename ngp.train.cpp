...
namespace ngp::train {
    ...
    std::expected<TrainStats, std::string> InstantNGP::train(const std::int32_t iters) {
        try {
            ...
            for (std::int32_t i = 0; i < iters; ++i) {
                ...
                cuda::evaluate_network(this->host.inference_sample_count, this->device.sample_coords, this->host.grid_offsets.data(), this->device.params, this->host.density_param_offset, this->host.global_color_param_offset, this->host.grid_param_offset, this->device.density_input, this->device.rgb_input, this->device.network_output);
    ...
    std::expected<TestStats, std::string> InstantNGP::test() const {
        try {
            ...
            const std::filesystem::path output_dir = this->host.options.test_output_dir;
            ...
            const std::uint32_t test_image_count = std::min(this->host.test_frame_count, this->host.options.test_frame_limit);
            const std::uint64_t pixel_count      = pixels_per_image * test_image_count;
            double evaluation_loss_sum           = 0.0;
            std::vector<std::uint8_t> comparison_image(static_cast<std::size_t>(this->host.test_width) * this->host.test_height * 2uz * 3uz);

            for (std::uint32_t image_index = 0u; image_index < test_image_count; ++image_index) {
            ...
            return TestStats{
                ...
                .comparison_image_count = test_image_count,
                .pixel_count            = pixel_count,
                .mse                    = static_cast<float>(mse),
                .psnr                   = mse > 0.0 ? static_cast<float>(-10.0 * std::log10(mse)) : std::numeric_limits<float>::infinity(),
    ...
    std::expected<void, std::string> InstantNGP::export_weights(const std::filesystem::path& path) const {
        try {
            ...
            const std::array tensors                  = std::to_array<SafetensorsTensor>({
                ...
                SafetensorsTensor{.name = "global_color.logits", .param_offset = this->host.global_color_param_offset, .rows = 3u, .cols = 1u},
            });
            ...
            metadata["format"]                    = "hyfluid-dynamic-instant-ngp.weights.v1";
            ...
            metadata["global_color_param_count"]  = std::format("{}", this->host.global_color_param_count);
    ...
    std::expected<void, std::string> InstantNGP::load_weights(const std::filesystem::path& path) {
        try {
            ...
            expected_metadata["format"]                    = "hyfluid-dynamic-instant-ngp.weights.v1";
            ...
            expected_metadata["global_color_param_count"]  = std::format("{}", this->host.global_color_param_count);
