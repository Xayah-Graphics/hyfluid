...
namespace hyfluid::cuda {
    ...
    // Sampler.
    void allocate_sampler_buffers(std::uint32_t occupancy_grid_bin_count, float*& out_sample_coords, float*& out_rays, std::uint32_t*& out_ray_indices, std::uint32_t*& out_numsteps, std::uint32_t*& out_ray_counter, std::uint32_t*& out_sample_counter, std::uint8_t*& out_occupancy, std::uint32_t*& out_occupancy_grid_bin_counts);
    void set_occupancy_grid_full(std::uint8_t* occupancy, std::uint32_t* occupancy_grid_bin_counts, std::uint32_t occupancy_grid_bin_count);
    void allocate_density_grid_buffers(std::uint32_t occupancy_grid_bin_count, float*& out_density_grid_values, float*& out_density_grid_scratch, std::uint32_t*& out_density_grid_indices, float*& out_density_grid_mean);
    bool update_density_grid_values(std::uint32_t current_step, std::uint32_t occupancy_grid_bin_count, const std::uint16_t* params, float* sample_coords, std::uint16_t* network_input, std::uint16_t* network_hidden, std::uint16_t* network_output, void* cublaslt_handle, std::uint8_t* cublaslt_workspace, float* density_grid_values, float* density_grid_scratch, std::uint32_t* density_grid_indices, std::uint32_t& density_grid_ema_step, std::uint32_t& out_updated_bin);
    void update_occupancy_grid_bank_from_density_grid(const float* density_grid_values, float* density_grid_mean, std::uint32_t* occupancy_grid_bin_counts, std::uint8_t* occupancy, std::uint32_t occupancy_grid_bin_count);
    void update_occupancy_grid_bin_from_density_grid(const float* density_grid_values, float* density_grid_mean, std::uint32_t* occupancy_grid_bin_counts, std::uint8_t* occupancy, std::uint32_t occupancy_grid_bin_count, std::uint32_t bin_index);
    void read_occupancy_grid_bin_counts(const std::uint32_t* occupancy_grid_bin_counts, std::uint32_t occupancy_grid_bin_count, std::uint32_t* host_counts);
    void sample_training_batch(const float* camera, const float* intrinsics, const std::uint32_t* frame_indices, const float* field_to_world_linear, std::uint32_t view_count, std::uint32_t time_count, std::uint32_t occupancy_grid_bin_count, std::uint32_t width, std::uint32_t height, std::uint32_t current_step, std::uint32_t rays_per_batch, std::uint32_t sample_limit, const std::uint8_t* occupancy, float* sample_coords, float* rays, std::uint32_t* ray_indices, std::uint32_t* numsteps, std::uint32_t* ray_counter, std::uint32_t* sample_counter);
